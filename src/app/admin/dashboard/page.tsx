    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`w-full flex items-center justify-between bg-surface-800/80 border ${isOpen ? 'border-primary-500 ring-1 ring-primary-500' : 'border-surface-600'} rounded-xl px-4 py-3 text-sm text-left transition-all shadow-inner ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-surface-500'}`}
            >
                <span className={selectedOption ? 'text-slate-200 truncate' : 'text-slate-500 truncate'}>
            ...
            {isOpen && (
                <div className="absolute z-50 w-full mt-2 bg-surface-800 border border-surface-600 rounded-xl shadow-2xl py-1 max-h-60 overflow-y-auto custom-scrollbar">
                    {options.length === 0 ? (
                        <div className="px-4 py-3 text-sm text-slate-500 italic text-center">No options available</div>
                    ) : (
                        options.map((option) => (
                            <button
                                key={option.value}
...
export default function AdminDashboardPage() {
    ...
    const [activeTab, setActiveTab] = useState<'schedules' | 'results' | 'users'>('schedules');
        ...
        <div className="min-h-screen bg-surface-900 text-slate-200 bg-fixed">
            ...
            <main className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <ModernSelect
                                        ...
                                        options={[
                                            { value: '', label: '-- Default Standard Pool --' },
                                            ...questionBanks.map(qb => ({ value: qb.id, label: qb.title }))
                                        ]}
                                    />
                                </div>
                            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* Tabs Navigation */}
                    <div className="flex bg-surface-800/80 rounded-2xl p-1 shadow-lg border border-surface-700 w-full mb-6">
                        <button onClick={() => setActiveTab('schedules')} className={`flex-1 py-3 text-sm font-semibold rounded-xl transition-all ${activeTab === 'schedules' ? 'bg-primary-500 shadow-lg shadow-primary-500/25 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-surface-700/50'}`}>⏱️ Schedules</button>
                        <button onClick={() => setActiveTab('results')} className={`flex-1 py-3 text-sm font-semibold rounded-xl transition-all ${activeTab === 'results' ? 'bg-secondary-500 shadow-lg shadow-secondary-500/25 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-surface-700/50'}`}>📊 Feedback</button>
                        <button onClick={() => setActiveTab('users')} className={`flex-1 py-3 text-sm font-semibold rounded-xl transition-all ${activeTab === 'users' ? 'bg-success-500 shadow-lg shadow-success-500/25 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-surface-700/50'}`}>👥 Users</button>
                    </div>
                                    ...
                                    <table className="w-full text-left text-sm whitespace-nowrap">
                                        <thead className="text-xs text-slate-400 uppercase bg-surface-800/50 border-b border-surface-700">
                                            <tr>
                                                ...
                                                <th className="px-5 py-4 font-semibold tracking-wider">Recording</th>
                                                <th className="px-5 py-4 font-semibold tracking-wider text-right">Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-surface-700/50">
                                            {results.length === 0 ? (
                                                ...
                                                <tr key={r.id} className={idx % 2 === 0 ? 'bg-transparent' : 'bg-surface-800/20'}>
                                                    ...
                                                    <td className="px-5 py-4">
                                                        {r.recording_url ? (
                                                            <a href={r.recording_url} target="_blank" rel="noopener noreferrer" className="text-primary-400 hover:underline flex items-center gap-1 text-xs">
                                                                <span>🎥</span> Watch
                                                            </a>
                                                        ) : (
                    ...
                    {activeTab === 'users' && (
                        ...
                        >
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                                    <span className="text-success-400">👥</span> Registered Users
                                </h2>
                                <span className="bg-surface-800 px-3 py-1 rounded-full text-xs font-medium text-slate-300 border border-surface-600">
                            ...
                            <div className="bg-surface-900/50 rounded-2xl border border-surface-700/50 overflow-hidden">
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-sm whitespace-nowrap">
                                        <thead className="text-xs text-slate-400 uppercase bg-surface-800/50 border-b border-surface-700">
                                            <tr>
                                                <th className="px-5 py-4 font-semibold tracking-wider">User ID</th>
                                                <th className="px-5 py-4 font-semibold tracking-wider">Email</th>
                                                <th className="px-5 py-4 font-semibold tracking-wider">Role</th>
                                                <th className="px-5 py-4 font-semibold tracking-wider">Registered On</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-surface-700/50">
                                            {profiles.length === 0 ? (
                                                ...
                                                <tr key={p.id} className={idx % 2 === 0 ? 'bg-transparent' : 'bg-surface-800/20'}>
                                                    <td className="px-5 py-4 text-xs font-mono text-slate-500" title={p.id}>{p.id.substring(0, 8)}...</td>
                                                    <td className="px-5 py-4 font-medium text-slate-200">{p.email}</td>
                                                    <td className="px-5 py-4">
                                                        <span className={`px-2.5 py-1 rounded-md text-xs font-bold ${p.role === 'admin' ? 'bg-accent-500/10 text-accent-400 border border-accent-500/20' :
